...
$(window).on("load", function () {
    ...
    function dollarFormatter(n) {
        ...
        var result = n;
        return (
            Math.abs(n) > 1e3 && (result = Math.round(n / 1e3) + "K"),
            "$" + result
        );
    }
    function resize() {
    ...
    d3.csv(
        ...
        function (error, data) {
            ...
            for (var cumulative = 0, i = 0; i < data.length; i++)
                (data[i].start = cumulative),
                    (cumulative += data[i].value),
                    (data[i].end = cumulative),
                    (data[i]["class"] =
                        data[i].value >= 0 ? "positive" : "negative");
            data.push({
                name: "Total",
                end: cumulative,
                start: 0,
