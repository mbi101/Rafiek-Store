...
$(window).on("load", function () {
    ...
    var d3CalendarGlobals = (function () {
        ...
        function publicDaysInMonth() {
            ...
            var firstDayOfTheWeek = new Date(
                publicYearToDisplay(),
                publicMonthToDisplay(),
                1
            ).getDay();
            ...
            for (i = 1; i <= firstDayOfTheWeek; i++) {
                daysArray.push([
                    daysInPreviousMonth - firstDayOfTheWeek + i,
                    cellColorForCurrentMonth,
                ]);
            }
    ...
    function renderDaysOfMonth(month, year) {
        $("#currentMonth").text(
            d3CalendarGlobals.monthToDisplayAsText() +
                " " +
                d3CalendarGlobals.yearToDisplay()
        );
        ...
        d3CalendarGlobals.datesGroup
            ...
            .attr(
                "transform",
                "translate(" +
                    d3CalendarGlobals.gridXTranslation +
                    "," +
                    d3CalendarGlobals.gridYTranslation +
                    ")"
            )
            .text(function (d) {
                return d[0];
    ...
    function drawGraphsForMonthlyData() {
        ...
        d3CalendarGlobals.chartsGroup.selectAll("g.arc").remove();

        var arcs = d3CalendarGlobals.chartsGroup
            .selectAll("g.arc")
            ...
            .attr("transform", function (d) {
                ...
                return (
                    "translate(" +
                    (outerRadius +
                        d3CalendarGlobals.gridXTranslation * 5 +
                        cellPositions[currentDataIndex][0]) +
                    ", " +
                    (outerRadius +
                        d3CalendarGlobals.gridYTranslation * 1.25 +
    ...
    function renderCalendarGrid(month, year) {
        ...
        d3CalendarGlobals.datesGroup =
            d3CalendarGlobals.calendar.append("svg:g");
        var daysInMonthToDisplay = d3CalendarGlobals.daysInMonth();
        d3CalendarGlobals.datesGroup
            .selectAll("daysText")
            .data(daysInMonthToDisplay)
