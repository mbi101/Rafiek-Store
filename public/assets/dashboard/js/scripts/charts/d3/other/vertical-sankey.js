...
$(window).on("load", function () {
    ...
    d3.json(
        "../../../app-assets/data/d3/other/sankey.json",
        function (error, energy) {
            if (error) throw error;

            sankey.nodes(energy.nodes).links(energy.links).layout(32); // what is this? iterations
            ...
            var link = svg
                ...
                .style("stroke", function (d) {
                    return (d.source.color = color(
                        d.source.name.replace(/ .*/, "")
                    ));
                })
                .style("fill", "none")
            ...
            function dragmove(d) {
                d3.select(this).attr(
                    ...
                    "translate(" +
                        (d.x = Math.max(
                            0,
                            Math.min(width - d.dy, d3.event.x)
                        )) +
                        "," +
                        d.y +
                        ")"
