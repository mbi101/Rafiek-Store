...
$(window).on("load", function () {
    function update(source) {
        ...
        var nodeUpdate = node
            ...
            .duration(duration)
            .attr("transform", function (d) {
                return (p = calcLeft(d)), "translate(" + p.y + "," + p.x + ")";
            });
        nodeUpdate
            .select("circle")
            .attr("r", 4.5)
    ...
    var tree = d3.layout.tree().size([height, width]),
        ...
        calcLeft = function (d) {
            var l = d.y;
            return (
                d.isRight || ((l = d.y - halfWidth), (l = halfWidth - l)),
                { x: d.x, y: l }
            );
        },
        toArray = function (item, arr) {
            arr = arr || [];
            var i = 0,
                l = item.children ? item.children.length : 0;
            for (arr.push(item); i < l; i++) toArray(item.children[i], arr);
            return arr;
        };
    d3.json(
        "../../../app-assets/data/d3/tree/bracket-tree.json",
        function (json) {
            (root = json), (root.x0 = height / 2), (root.y0 = width / 2);
            var t1 = d3.layout
                    .tree()
            ...
            t1.nodes(root), t2.nodes(root);
            var rebuildChildren = function (node) {
                (node.children = getChildren(node)),
                    node.children && node.children.forEach(rebuildChildren);
            };
            rebuildChildren(root), (root.isRight = !1), update(root);
        }
    );
});
