...
!(function (window, document, $) {
    ...
    $(".select-contact").selectize({
        ...
        createFilter: function (input) {
            var regexpA = new RegExp("^" + REGEX_EMAIL + "$", "i"),
                regexpB = new RegExp("^([^<]*)<" + REGEX_EMAIL + ">$", "i");
        ...
        create: function (input) {
            ...
            if (match) {
                var name = $.trim(match[1]),
                    pos_space = name.indexOf(" "),
                    first_name = name.substring(0, pos_space),
                    last_name = name.substring(pos_space + 1);
                return {
                    email: match[2],
    ...
    $(".repositories").selectize({
        ...
        $(".selectize-locked").selectize({ create: !0 }),
        $(".selectize-locked")[0].selectize.lock(),
        $(".selectize-sort").selectize({ sortField: "text" });
    for (
        ...
        for (var title = [], j = 0; j < 8; j++)
            title.push(
                letters.charAt(Math.round((letters.length - 1) * Math.random()))
            );
    ...
    $(".selectize-junk").selectize({
        ...
        $(".selectize-movie").selectize({
            ...
            render: {
                option: function (item, escape) {
                    for (
                        var actors = [], i = 0, n = item.abridged_cast.length;
                        i < n;
                        i++
                    )
                        actors.push(
                            "<span>" +
                                escape(item.abridged_cast[i].name) +
                                "</span>"
                        );
                    return (
                        '<div><img src="' +
        ...
        $(".selectize-rtl-input").selectize({ persist: !1, create: !0 }),
